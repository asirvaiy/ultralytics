...
class TaskAlignedAssigner(nn.Module):
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        count_tensor = torch.zeros(metrics.shape, dtype=torch.int, device=topk_idxs.device)
        ones = torch.ones_like(topk_idxs[:, :, :1], dtype=torch.int, device=topk_idxs.device)
